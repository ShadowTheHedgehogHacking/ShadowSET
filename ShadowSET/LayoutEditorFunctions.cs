        ...
        public static List<SetObjectHeroes> GetHeroesLayout(string fileName, out string result, ref Dictionary<(byte, byte, string), HashSet<byte[]>> miscSettingsDict)
        {
            ...
            using (var reader = new EndianBinaryReader(new FileStream(fileName, FileMode.Open), Endianness.Big))
                for (int i = 0; i < HeroesObjectCount; i++)
                {
                    ...
                    var obj = CreateHeroesObject(List, Type, PosX, PosY, PosZ, RotX, RotY, RotZ, /* pos, rot,*/ Link, Rend, unkBytes1.Concat(unkBytes2).ToArray(), false);
...
/*# if DEBUG
        ...
        public static List<SetObjectShadow> GetShadowLayout(string fileName, out string result)
        {
            ...
            for (int i = 0; i < count; i++)
            {
                ...
                list.Add(CreateShadowObject(List, Type, PosX, PosY, PosZ, RotX, RotY, RotZ, /*pos, rot,*/  Link, Rend, UnkBytes, false));
            }
...
/*#if DEBUG
            ...
            foreach (string j in SplineFile)
            {
                if (j.StartsWith("v"))
                {
                    ...
                    SetObjectHeroes heroesSetObject = CreateHeroesObject
                        (List, Type, /* new Vector3(Convert.ToSingle(a[1]), Convert.ToSingle(a[2]), Convert.ToSingle(a[3])), */Convert.ToSingle(a[1]), Convert.ToSingle(a[2]), Convert.ToSingle(a[3]), /* Vector3.Zero ,*/ 0, 0, 0, 0, 10);
        ...
        public static List<SetObjectHeroes> GetHeroesLayoutFromINI(string fileName)
        {
            ...
            SetObjectHeroes temp = CreateHeroesObject(0, 0, /* Vector3.Zero, Vector3.Zero,*/ 0, 0, 0, 0, 0, 0, 0, 10, new byte[8], false);
            ...
            foreach (string s in file)
            {
                if (s.StartsWith("obj "))
                {
                    ...
                    temp = null;
                    temp = CreateHeroesObject(Convert.ToByte(s.Substring(4, 2), 16), Convert.ToByte(s.Substring(6, 2), 16), /* Vector3.Zero, Vector3.Zero, */ 0, 0, 0, 0, 0, 0, 0, 10, createMatrix: false);
                }
                ...
                {
                    string[] j = s.Split(' ');
                    //new Vector3(Convert.ToSingle(j[1]), Convert.ToSingle(j[2]), Convert.ToSingle(j[3]));
                    temp.PosX = Convert.ToSingle(j[1]);
                    temp.PosY = Convert.ToSingle(j[2]);
                    temp.PosZ = Convert.ToSingle(j[3]);
                }
                else if (s.StartsWith("r "))
                {
                    string[] j = s.Split(' ');
                    // temp.Rotation = new Vector3(Convert.ToSingle(j[1]), Convert.ToSingle(j[2]), Convert.ToSingle(j[3]));
                    temp.RotX = Convert.ToSingle(j[1]);
                    temp.RotY = Convert.ToSingle(j[2]);
                    temp.RotZ = Convert.ToSingle(j[3]);
                }
        ...
        public static void SaveHeroesLayoutINI(IEnumerable<SetObjectHeroes> list, string outputFile)
        {
            ...
            foreach (var obj in list)
            {
                ...
                iniWriter.WriteLine("v " + obj.PosX.ToString() + " " + obj.PosY.ToString() + " " + obj.PosZ.ToString());
                iniWriter.WriteLine("r " + obj.RotX.ToString() + " " + obj.RotY.ToString() + " " + obj.RotZ.ToString());
        ...
        public static SetObjectHeroes CreateHeroesObject
            (byte List, byte Type, /*Vector3 Position, Vector3 Rotation,*/ float PosX, float PosY, float PosZ, float RotX, float RotY, float RotZ, byte Link, byte Rend, byte[] UnkBytes = null, bool createMatrix = true)
        {
            var objEntry = LayoutEditorSystem.heroesObjectEntry(List, Type);
            // TODO: Restore when Heroes obj definitions are added
            SetObjectHeroes heroesObj = new SetObjectHeroes();
...
/*            if (createMatrix)
        ...
        private static SetObjectHeroes SetObjectShadowToHeroes(SetObjectShadow i)
        {
            ...
            if (MatchNotFound)
                ...
                return CreateHeroesObject(List, Type, i.PosX, i.PosY, i.PosZ, DegreesToBAMS(i.RotX), DegreesToBAMS(i.RotY), DegreesToBAMS(i.RotZ), 0, (byte)(2 * i.Rend));
