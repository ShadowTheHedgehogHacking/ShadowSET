...
/*        public void SelectedIndexChanged(ListBox.SelectedIndexCollection selectedIndices)
        ...
        public void ChangeObjectType(int index, ObjectEntry newEntry)
        {
            ...
            var posX = current.PosX;
            var posY = current.PosY;
            var posZ = current.PosZ;
            var rotX = current.RotX;
            var rotY = current.RotY;
            var rotZ = current.RotZ;
            ...
            setObjects[index] = isShadow ?
                CreateShadowObject(newEntry.List, newEntry.Type, /* pos, rot,*/ posX, posY, posZ, rotX, rotY, rotZ, link, rend, unkb) :
                CreateHeroesObject(newEntry.List, newEntry.Type, /* pos, rot, */ posX, posY, posZ, rotX, rotY, rotZ, link, rend, unkb);
