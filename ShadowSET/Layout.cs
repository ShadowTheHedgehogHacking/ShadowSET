...
namespace ShadowSET
{
	static class Layout
	{
		public static List<SetObjectShadow> GetShadowLayout(string fileName)
		{
			...
			if (FileMagic != "sky2")
			{
				throw new InvalidDataException("Not a valid Shadow the Hedgehog layout file");
			}
			...
			for (int i = 0; i < AmountOfObjects; i++)
			{
				...
				float PosX = LayoutFileReader.ReadSingle();
				float PosY = LayoutFileReader.ReadSingle();
				float PosZ = LayoutFileReader.ReadSingle();

				float RotX = LayoutFileReader.ReadSingle();
				float RotY = LayoutFileReader.ReadSingle();
				float RotZ = LayoutFileReader.ReadSingle();
				...
				SetObjectShadow TempObject = CreateShadowObject(List, Type, PosX, PosY, PosZ, RotX, RotY, RotZ, Link, Rend, UnkBytes);
		...
		public static SetObjectShadow CreateShadowObject
			(byte List, byte Type, float PosX, float PosY, float PosZ, float RotX, float RotY, float RotZ, byte Link, byte Rend, byte[] UnkBytes = null)
		{
